% tic toc
for i = 1:20
    im = imread(['TennisSet1/stennis.' int2str(i),'.ppm']);
    ...
    % bin2 = edge(noiseIm2,'canny', [0.1 0.2]);

%     figure('Position', [350 400 900 600]);
%     imshow(noiseIm2, 'InitialMagnification',250);

    figure;
    imshow(bin2, 'InitialMagnification',250);
    ...
    % c = bwpropfilt(bin2,'eccentricity',[0.9, 0.995]); %[0.8, 0.9] for set 2?
...
%     h = bwareafilt(e,[100, 400]); %150 - 400
%     % To only keep the ball 
%     i = bwareafilt(h,1,'smallest');
% 

...
for imageID = 1:length(xyCentreBall)
...
%                 fprintf('numberOfGreenBlobs: %d\n',numberOfGreenBlobs);  
                for f = 1:numberOfObjects
                    ...
                    heuristicTemp = sqrt(double((xyCentreBallTemp(imageID,2) - ...
                                    xyCentreBall(imageID-1,2))^2 + ...
                                    (xyCentreBallTemp(imageID,1) - ...
                                    xyCentreBall(imageID-1,1))^2));
...
%                         fprintf('Area: %d\n',uint16(propsCube(k).Area));
                ...
                if (heuristic > 30)
                    xyCentreBallNext(imageID,1) = xyCentreBall(imageID-1,1);
                    xyCentreBallNext(imageID,2) = xyCentreBall(imageID-1,2);
                end
                
                 xyCentreBall(imageID,1) = xyCentreBallNext(imageID,1);
                 xyCentreBall(imageID,2) = xyCentreBallNext(imageID,2);
...
% % % %         fprintf('Ball Position: %d   %d\n',xyCentreBall(imageID,1), xyCentreBall(imageID,2));    
...
%     end
    ...
    %% To isolate the Bat
        ...
        sizeIm = bwareafilt(erodedBW,[30, 1200]);
        
        areaIm = bwpropfilt(sizeIm,'Area',12);
